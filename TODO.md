...
- **Media E2EE has a key nobody can distribute yet.** `DISCORDIA_E2EE_KEY` has
  to be typed into every participant's environment by hand, which is fine for
  proving the mechanism and useless as a feature. The design that makes it real:
  a per-channel key generated by a member, encrypted once per recipient to their
  Nostr pubkey (ECDH — the identity keys already exist and the roadmap already
  wants NIP-44 for DMs), and passed through the server as blobs it cannot read.
  Needs a protocol message, client-side crypto, and key state in `AppState`.
- **And it must rekey on kick and ban.** Without it a removed member keeps
  decrypting whatever they can still capture off the wire, which makes the
  moderation actions look effective while they are not. The rekey has to reach
  all four connections a member holds, so it belongs with distribution rather
  than after it.
