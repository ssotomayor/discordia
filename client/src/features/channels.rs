    ...
    rsx! {
        div { class: "border-t border-[var(--border)]",
                ...
                div { class: "px-3 py-2 border-b border-[var(--border)]",
                    div { class: "flex items-center gap-2",
                        ...
                        button {
                            ...
                            onclick: move |_| {
                                let now = !sharing;
                                // Only act when we're actually in a voice
                                // channel: the screen room is keyed off it and
                                // the JS bridge needs the ScreenToken that
                                // JoinVoice produced. Without a channel, leave
                                // local state untouched to avoid desync.
                                if let Some(cid) = voice_channel {
                                    ...
                                    // Starting must run inside this click
                                    // gesture so getDisplayMedia is allowed;
                                    // stop is gesture-agnostic.
            ...
            div { class: "h-12 px-3 flex items-center gap-2",
                ...
                // as the activity / screen-share windows).
                if show_audio_settings() {
                    // Dismiss overlay: a transparent full-viewport layer that
                    // sits *under* the popover (z-30 < z-40) but *over* the rest
                    // of the app, so the first click outside the popover closes
                    // it. The drag overlay above is z-50, so this never
                    // intercepts an in-flight drag.
                    div {
