...
> **Status: in use, pre-release.** Drag, resize, collision resolution and the
> `use_layout_store` hook all ship — see `drag.rs`, `collision.rs`, `store.rs`.
> The consumer inside this workspace drives the free-floating mode; the
> snap-to-grid path is exercised only by the example, so treat it as the less
> travelled of the two.
...
| Version | Adds |
|---|---|
| **0.0.1** ✅ | Static rendering. `GridLayout`/`GridItem` props, CSS-grid placement, `pinned` flag, serializable `GridPosition` / `FloatRect`. |
...
For now this crate lives inside the [`discordia`](https://github.com/ssotomayor/discordia)
